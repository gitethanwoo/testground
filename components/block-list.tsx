...
import { GripVertical, MessageSquare, Wand2 } from "lucide-react";
...
import { BlockControls } from "./block/block-controls";
...
export function BlockList({ blocks, onBlocksChange, openSettingsId, setOpenSettingsId, executeBlock }: BlockListProps) {
    const { updateBlockName, toggleBlockExpanded, removeBlock, updateBlockSettings, getAvailableVariables, onDragEnd } =
    ...
    return (
        <DragDropContext onDragEnd={onDragEnd}>
            <Droppable droppableId="blocks">
                {provided => (
                    <div {...provided.droppableProps} ref={provided.innerRef} className="relative space-y-4 py-2">
                        {blocks.length > 1 && (
                      ...
                      ${snapshot.isDragging ? "shadow-xl ring-2 ring-blue-100" : ""}`}
                        ...
                        transition-colors duration-200`}
                            ...
                            ${block.type === "Input" ? "bg-purple-100 text-purple-700" : "bg-amber-100 text-amber-700"
                                                            }`}
                                                    >
